...
- **`duidtin-ui/`** (nanti, port 3000) — host: shell, routing, consumer semua remote.
...
- **`duidtin-ui-layout/`** (port 3002) — header/footer, layout bersama di tiap halaman. Sudah jalan, dan sudah kebukti konsumsi komponen dari design-system lewat `loadRemote()`.
...
Yang tinggal: host `duidtin-ui`. Dua remote di atas sekarang jalan berpasangan (`duidtin-ui-layout` jadi konsumen pertama design-system), jadi jalur MF-nya sudah teruji beneran di browser sebelum host-nya dibikin.
