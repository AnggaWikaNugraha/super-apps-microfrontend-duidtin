...
- File `.css` juga di-exclude dari entri pertama (`"!./src/**/*.css"`) — karena `bundle: false` bikin tiap file diproses **sendiri-sendiri**, file CSS per komponen bakal ketemu PostCSS tanpa konteks `@import "tailwindcss" prefix(ui)` dari `index.tailwind.css`, dan `@apply ui:...`-nya langsung error (`Cannot apply utility class 'ui:inline-flex' because the 'ui' variant does not exist`). CSS-nya digabung & dikompilasi di entri kedua. Hasil copy per-file di `dist/` itu juga nggak pernah dipakai — `exports` cuma nunjuk `dist/index.tailwind.css` lewat field `"./css"`.

## `postcss.config.mjs` `packages/ui`

Cuma satu isinya: plugin `@tailwindcss/postcss`. Tanpa file ini, `rslib build` nyalin `@apply ui:...` mentah-mentah ke `dist/index.tailwind.css` dan browser konsumen mengabaikannya — komponen kerender tapi polos. Storybook nggak kena karena dia compile Tailwind sendiri lewat `@tailwindcss/vite` di `.storybook/main.ts`, jalur yang beda total dari `rslib build`. Ini alasannya bug-nya baru ketahuan waktu `duidtin-ui-layout` nge-`loadRemote` komponennya.
